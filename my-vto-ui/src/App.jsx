...
const Sidebar = ({ page, setPage, user, onLogout, onSignup, onOpenCreditHistory }) => (
  (() => {
    const lowerNavOrder = ["pricing", "settings", "guide"];
...
const STYLE_OPTIONS = [
  ...
  { id: "ghost", label: "ゴースト", sub: "Ghost", icon: "◌", desc: "ゴーストマネキン風に立体感を保って生成します", credit: 2, previewImage: "/ghost.png" },
  { id: "model", label: "モデル", sub: "Model", icon: "◉", desc: "実写風モデル画像を生成します（方式により 1cr または 4cr）", credit: 1 },
  { id: "custom", label: "カスタムプロンプト", sub: "Custom Prompt", icon: "✎", desc: "自分で細かい描写を設定して生成します", credit: 3 },
];
const OUTPUT_RATIO_OPTIONS = [
  { id: "fourThree", label: "4:3（デフォルト）" },
  { id: "oneOne", label: "1:1" },
...
const DEMO_ALLOWED_ORIENTATIONS_BY_SAMPLE = {
  demo_sample_1: ["front", "front45"],
  demo_sample_2: ["front", "front45"],
  demo_sample_3: ["front", "front45"],
  demo_sample_4: ["back", "back45"],
};
...
const DEFAULT_STUDIO_ASSETS = [
  { id: "bg1", name: "標準スタジオ 1", outputUrl: "/bg3.png", builtIn: true, favorite: false },
  { id: "bg2", name: "標準スタジオ 2", outputUrl: "/bg7.png", builtIn: true, favorite: false },
  { id: "bg3", name: "標準スタジオ 3", outputUrl: "/bg1.png", builtIn: true, favorite: false },
  { id: "bg4", name: "標準スタジオ 4", outputUrl: "/bg4.png", builtIn: true, favorite: false },
  { id: "bg5", name: "標準スタジオ 5", outputUrl: "/bg5.png", builtIn: true, favorite: false },
  { id: "bg6", name: "標準スタジオ 6", outputUrl: "/bg6.png", builtIn: true, favorite: false },
  { id: "bg7", name: "標準スタジオ 7", outputUrl: "/bg2.png", builtIn: true, favorite: false },
  { id: "bg8", name: "標準スタジオ 8", outputUrl: "/bg8.png", builtIn: true, favorite: false },
  { id: "bg9", name: "標準スタジオ 9", outputUrl: "/bg9.png", builtIn: true, favorite: false },
  { id: "bg10", name: "標準スタジオ 10", outputUrl: "/bg10.png", builtIn: true, favorite: false },
];
...
const DEFAULT_MODEL_ASSETS = [
  { id: "mdl_m01", name: "標準モデル 1", outputUrl: "/m0.1.png", builtIn: true, favorite: false },
  { id: "mdl_m02", name: "標準モデル 2", outputUrl: "/m0.2.png", builtIn: true, favorite: false },
  { id: "mdl_m1", name: "標準モデル 3", outputUrl: "/m1.png", builtIn: true, favorite: false },
  { id: "mdl_m2", name: "標準モデル 4", outputUrl: "/m2.png", builtIn: true, favorite: false },
  { id: "mdl_m3", name: "標準モデル 5", outputUrl: "/m3.png", builtIn: true, favorite: false },
  { id: "mdl_m4", name: "標準モデル 6", outputUrl: "/m4.png", builtIn: true, favorite: false },
  { id: "mdl_m5", name: "標準モデル 7", outputUrl: "/m5.png", builtIn: true, favorite: false },
  { id: "mdl_m6", name: "標準モデル 8", outputUrl: "/m6.png", builtIn: true, favorite: false },
  { id: "mdl_m7", name: "標準モデル 9", outputUrl: "/m7.png", builtIn: true, favorite: false },
];
...
const BACKEND_PREVIEW_BASE_URL = (import.meta.env.VITE_BACKEND_BASE_URL || "http://localhost:8787").replace(/\/$/, "");
...
async function persistProductAssetsToDb(userId, products = []) {
  ...
  try {
    ...
    (products || []).forEach((asset) => {
      ...
      store.put({
        key: productAssetKey(userId, asset.id),
        userId,
        assetId: asset.id,
        dataUrl,
...
async function buildCoordinateCompositeFile({ topDataUrl, bottomDataUrl, shoesDataUrl = "" }) {
  ...
  return new File([blob], `coordinate-${Date.now()}.png`, {
    type: "image/png",
...
async function buildCoordinateCompositeFromList(dataUrls = []) {
  ...
  if (images.length === 1) {
    ...
    return new File([blobSingle], `coordinate-${Date.now()}.png`, {
      type: "image/png",
      lastModified: Date.now(),
    });
  }
...
function mergeDefaultModelAssets(modelAssets = []) {
  ...
  const legacyDefaultUrls = new Set(["/mannequin.png", "/ghost.png"]);
...
function UploadPage({ user, jobs, onDataRefresh, studioAssets = [], modelAssets = [], productAssets = [] }) {
  ...
  const [outputPreset, setOutputPreset] = useState("fourThree");
  ...
  const outputPresetLabel = (OUTPUT_RATIO_OPTIONS.find((opt) => opt.id === outputPreset)?.label) || "4:3（デフォルト）";
  const activeJob = jobs.find((job) => job.id === currentJobId) || null;
  const hasInFlightJob = jobs.some((job) => job.status === "queued" || job.status === "processing");
  ...
  const selectedInputFiles = useMemo(() => {
    if (!user?.isDemo) return files;
    return files.filter((entry) => entry.source === "sample" && entry.id === selectedDemoInputId);
  }, [files, selectedDemoInputId, user?.isDemo]);
  ...
  const effectiveBackgroundReference = usePromptBackground ? "" : (selectedBackground?.outputUrl || selectedBackground?.dataUrl || "");
  ...
  useEffect(() => {
    if (!requiresModelReference && modelAssetId) {
      setModelAssetId("");
    }
  }, [requiresModelReference, modelAssetId]);
  ...
  }, [customPrompt, forceTryonV16Basic, framing, generationQuality, modelDevPipeline, modelRunStrategy, orientation, style]);

  useEffect(() => {
    if (!user?.isDemo) return;
    if (!requiresModelReference) return;
  ...
  }, [modelAssetId, modelAssets, requiresModelReference, user?.isDemo]);
  ...
  useEffect(() => {
    if (style === "torso" || style === "mannequin" || style === "model") {
      setFraming("focus");
      return;
    }
    setFraming("full");
  ...
  const qualityExtra = outputQuality === "high"
    ...
    ? 1
    : 0;
  const modelReferenceExtra = modelAssetId && activeStyle !== "model" && effectiveModelStrategy !== "tryon-max" ? 1 : 0;
  ...
  const run = async () => {
    ...
    if (effectiveBackgroundMode === "image" && !effectiveBackgroundReference) {
    ...
    try {
                      ...
                      : outputPreset === "twoThree" ? "2:3"
                        : outputPreset === "threeTwo" ? "3:2"
                          : outputPreset === "fiveFour" ? "5:4"
                            : "4:3",
        background: {
          type: effectiveBackgroundMode === "image" ? "studio" : "solid",
      ...
      if (!user?.isDemo) {
        ...
        if (!hasZip && inputEntries.length > 1) {
      ...
      const job = await createJob({
        ...
        modelReference: selectedModel?.outputUrl || selectedModel?.dataUrl || "",
        faceReference: "",
        modelRunStrategy,
        forceTryonV16Basic: style === "model" && forceTryonV16Basic,
        useModelImagePrompt: style === "model" && modelDevPipeline === "product-to-model-model",
        backgroundReference: effectiveBackgroundMode === "image" ? effectiveBackgroundReference : "",
        customPrompt: customPrompt.trim(),
  ...
  return (
    <div className="fade-up">
      <div style={{ marginBottom: 36 }}>
        ...
        <h1 style={{ fontFamily: SERIF, fontSize: 38, fontWeight: 400, color: C.text, lineHeight: 1.15 }}>
          服の写真を、<em style={{ fontStyle: "italic" }}>プロ品質</em>に変換する
        </h1>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 2, padding: 20 }}>
            ...
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.borderLight}` }}>
              ...
              <div style={{ display: "grid", gap: 7 }}>
                {ORIENTATION_OPTIONS.map((opt) => (
                  ...
                  <button
                    key={opt.id}
                    onClick={() => { if (!lockedInDemo) setOrientation(opt.id); }}
                    disabled={lockedInDemo}
                    style={{
                      width: "100%",
                      textAlign: "left",
                      border: `1px solid ${orientation === opt.id ? C.goldBorder : C.border}`,
                      background: orientation === opt.id ? C.goldLight : C.bg,
                      color: orientation === opt.id ? C.text : C.textSub,
                      fontSize: 11,
                      padding: "9px 10px",
                      cursor: lockedInDemo ? "not-allowed" : "pointer",
                      opacity: lockedInDemo ? 0.45 : 1,
                    }}
                  >
                    {orientation === opt.id ? "◉ " : "○ "}
                    {opt.label}
            ...
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.borderLight}` }}>
                ...
                <div style={{ display: "flex", gap: 8, opacity: usePromptBackground ? 0.45 : 1 }}>
                  <button
                    onClick={() => { if (!usePromptBackground) setBackgroundMode("solid"); }}
                    style={{
                      flex: 1,
                  ...
                  <button
                    onClick={() => { if (!usePromptBackground) setBackgroundMode("image"); }}
                    style={{
                      flex: 1,
                      border: `1px solid ${backgroundMode === "image" ? C.goldBorder : C.border}`,
                      background: backgroundMode === "image" ? C.goldLight : C.bg,
                      color: backgroundMode === "image" ? C.text : C.textSub,
                      fontSize: 11,
                      padding: "8px 6px",
                      cursor: usePromptBackground ? "not-allowed" : "pointer",
                    }}
                ...
                {!usePromptBackground && user?.isDemo && backgroundMode === "image" && (
                  <p style={{ fontSize: 10, color: C.red, lineHeight: 1.5 }}>
                    デモ版では{DEMO_ALLOWED_BACKGROUND_IDS.size}つの背景画像からお選びください。
                  </p>
                )}
                {!user?.isDemo && !usePromptBackground && backgroundMode === "image" && (
                ...
                <p style={{ fontSize: 10, color: C.textSub, lineHeight: 1.5 }}>
                  ...
                  {favoriteOnlyBackground && favoriteBackgrounds.length === 0 ? "（お気に入りがないため全背景を表示中）" : ""}
                </p>
              </div>
            </div>
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.borderLight}` }}>
              <p style={{ fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: C.textSub, marginBottom: 10 }}>縦横比率</p>
              <select
                value={outputPreset}
              ...
              >
                {OUTPUT_RATIO_OPTIONS.map((ratio) => (
                  <option key={ratio.id} value={ratio.id} disabled={Boolean(user?.isDemo && ratio.id !== "fourThree")}>
                    {ratio.label}
                  </option>
                ))}
              </select>
          ...
          <Btn
            ...
            size="lg"
            onClick={run}
              disabled={!generationInputReady || running || hasInFlightJob || isSubmitting || submitLocked || (user?.isDemo && activeStyle === "custom") || (!user?.isDemo && activeStyle === "custom" && !customPrompt.trim()) || (!user?.isDemo && activeStyle === "model" && !modelAssetId)}
          >
            {(running || hasInFlightJob || isSubmitting) ? (
          ...
          {files.length > 0 && (
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 2, padding: 20 }}>
              ...
              {[
                ...
                ["モデル参照", requiresModelReference ? (selectedModel ? selectedModel.name : "未選択") : "利用しない"],
              ...
              <div style={{ flex: 1, minHeight: 0, padding: 12, overflowY: "auto", overflowX: "hidden", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10, alignContent: "start", gridAutoRows: "max-content" }}>
                {lookFilteredAssets.map((asset) => (
                  ...
                  >
                    ...
                    <div style={{ padding: "6px 7px" }}>
                      <p style={{ fontSize: 10, color: C.textSub, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{asset.name}</p>
                    </div>
                  </button>
                ))}
              </div>
...
// PAGE: HISTORY
// ─────────────────────────────────────────────
function HistoryPage({ user, jobs, onRefresh }) {
  ...
  const totalCredits = jobs.reduce((sum, job) => sum + job.creditUsed, 0);
  const monthlyRuns = jobs.filter((job) => job.createdAt.startsWith(monthPrefix)).length;
  const doneRuns = jobs.filter((job) => job.status === "done").length;
  const doneRate = jobs.length ? Math.round((doneRuns / jobs.length) * 100) : 0;
  ...
  const selectAllImages = useCallback(() => {
    setSelectedImageIds(selectableItems.map((item) => item.id));
  }, [selectableItems]);
  ...
  const saveTargets = useCallback(async (targets) => {
    ...
    try {
      if (targets.length === 1) {
        ...
        await downloadImage(item.outputUrl, item.outputName || item.name || `${item.id}.png`, saveFormat);
        return;
      }
      await downloadImagesZip(targets, saveFormat);
    } catch (e) {
  ...
  const saveSelectedImages = useCallback(async () => {
    const targets = selectableItems.filter((item) => selectedImageIds.includes(item.id));
    await saveTargets(targets);
  }, [saveTargets, selectableItems, selectedImageIds]);
  ...
  const renderGeneratedCard = useCallback((item) => (
    <div
      key={item.id}
      onClick={() => {
        const absoluteIndex = generatedItems.findIndex((g) => g.id === item.id);
        openViewer(generatedItems, Math.max(0, absoluteIndex), "生成画像一覧");
      }}
      style={{
    ...
    >
      ...
      <div style={{ padding: "6px 8px" }}>
        ...
        <p style={{ fontSize: 10, color: C.textSub }}>Job: {item.jobId}</p>
      </div>
    </div>
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
        {[
          { label: "総生成件数", value: `${totalItems}`, unit: "件" },
          { label: "総クレジット使用", value: `${totalCredits}`, unit: "cr" },
          { label: "完了率", value: `${doneRate}`, unit: "%" },
          { label: "今月の生成回数", value: `${monthlyRuns}`, unit: "回" },
        ].map((s) => (
          <div key={s.label} style={{
            background: C.surface, border: `1px solid ${C.border}`,
            borderRadius: 2, padding: "18px 20px",
          }}>
            <p style={{ fontSize: 10, color: C.textSub, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>{s.label}</p>
            <p style={{ fontFamily: JP, fontSize: 30, fontWeight: 700, color: C.text, fontVariantNumeric: "tabular-nums" }}>
              {s.value}<span style={{ fontSize: 14, fontFamily: SANS, color: C.textSub, fontWeight: 300, marginLeft: 4 }}>{s.unit}</span>
            </p>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 2, padding: 18, marginBottom: 24, position: "relative" }}>
          ...
          <div ref={deleteConfirmRef} style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto", position: "relative", flexWrap: "nowrap" }}>
            {selectionMode && (
              <>
                ...
                >
                  <option value="png">PNGで保存</option>
                  <option value="jpg">JPGで保存</option>
                </select>
                <Btn
                  size="sm"
                  variant="primary"
        ...
        {generatedItems.length > 0 ? (
          ...
          <p style={{ fontSize: 12, color: C.textSub }}>まだ生成完了画像がありません。</p>
        )}
      </div>
                          ...
                          <div style={{ aspectRatio: "3/4", background: C.borderLight, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {item.outputUrl ? (
                              <button
                                ...
                                style={{ display: "block", width: "100%", height: "100%", padding: 0, margin: 0, border: "none", background: "transparent", cursor: "zoom-in" }}
                              >
                                <img src={item.outputUrl} alt={item.outputName || item.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                              </button>
                            ) : (
...
// PAGE: EDIT
// ─────────────────────────────────────────────
function EditPage({ jobs, user, studioAssets = [], modelAssets = [] }) {
  const EDIT_TABS = [
    { id: "background", label: "背景変更" },
    { id: "pose", label: "ポーズ変更" },
    { id: "model", label: "モデル変更" },
    { id: "face", label: "顔変更" },
    { id: "look", label: "ルック微調整" },
  ];
  ...
  const promptPlaceholderByTab = {
    background: "例: 背景を夕方の柔らかい光に変更。服・モデルは変更しない。",
    pose: "例: 立ち姿のまま右肩を少し引いて自然なS字ポーズに。",
    model: "例: モデルの雰囲気を落ち着いた大人系に。",
    face: "例: 表情を自然な微笑みにして目線はカメラへ。",
    look: "例: 全体の印象を高級EC向けに整える。",
  };
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 360px", gap: 18 }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, padding: 14 }}>
          <p style={{ fontSize: 11, color: C.textSub, marginBottom: 10 }}>編集パネル</p>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            {EDIT_TABS.map((tab) => (
              <button
                ...
                style={{
                  border: `1px solid ${activeEditTab === tab.id ? C.goldBorder : C.border}`,
                  background: activeEditTab === tab.id ? C.goldLight : C.surface,
                  color: activeEditTab === tab.id ? C.text : C.textSub,
                  fontSize: 11,
                  padding: "7px 10px",
                  cursor: "pointer",
                }}
          ...
          <textarea
            ...
            rows={8}
            placeholder={promptPlaceholderByTab[activeEditTab] || "編集内容を入力してください。"}
            style={{
              width: "100%",
              border: `1px solid ${C.border}`,
              background: C.bg,
          ...
          <p style={{ marginTop: 8, fontSize: 11, color: C.textSub, lineHeight: 1.6 }}>
            例: モデル画像のポーズ変更 / 背景画像の時間帯変更（昼→夕方）など、意図を具体的に入力してください。
          </p>
          <div style={{ marginTop: 12 }}>
            <Btn variant="primary" full disabled={!selectedImage || !editPrompt.trim()}>
              編集を実行（次対応）
            </Btn>
          </div>
        </div>
          ...
          {assets.map((asset, idx) => (
            <div key={asset.id} style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              <button
                ...
                style={{ aspectRatio: "3/4", background: C.bg, border: "none", padding: 0, width: "100%", display: "block", cursor: "default" }}
              >
              ...
              <div style={{ padding: "8px 9px", borderTop: `1px solid ${C.borderLight}` }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                  ...
                  {asset.builtIn && (
                    <span style={{ fontSize: 10, color: C.textSub }}>標準</span>
                  )}
                </div>
              </div>
            </div>
      ...
      {assets.length > 0 ? (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: 12 }}>
            ...
            <div key={asset.id} style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              <button
                onClick={() => openViewer(assets.findIndex((v) => v.id === asset.id))}
                style={{ aspectRatio: "3/4", background: C.bg, border: "none", padding: 0, width: "100%", display: "block", cursor: "pointer" }}
              >
                <img src={asset.outputUrl || asset.dataUrl || ""} alt={asset.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              </button>
              <div style={{ padding: "8px 9px", borderTop: `1px solid ${C.borderLight}` }}>
                <p style={{ fontSize: 10, color: C.textSub, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", marginBottom: 6 }}>
                  {asset.name}
                </p>
                <select
                  value={asset.category || "unassigned"}
                  onChange={(e) => updateCategory(asset.id, e.target.value)}
                  style={{ width: "100%", padding: "6px 8px", border: `1px solid ${C.border}`, background: C.bg, color: C.text, fontSize: 11, marginBottom: 6 }}
                >
      ...
      {viewer && viewer.items.length > 0 && typeof document !== "undefined" && createPortal((
        ...
        >
          ...
          >
            ...
            <div style={{ position: "relative", flex: 1, minHeight: 0, background: C.bg, overflow: "auto" }}>
              <div
                style={{
                  minHeight: viewerZoom > 1 ? "auto" : "100%",
                  display: "flex",
                  flexDirection: "column",
                  alignItems: "flex-start",
                  justifyContent: viewerZoom > 1 ? "flex-start" : "center",
                  gap: 10,
                  padding: "18px 16px",
                }}
              >
                <img
                  ...
                  style={{
                    ...
                    maxHeight: viewerZoom > 1 ? "none" : "72vh",
                    objectFit: "contain",
                    width: viewerZoom > 1 ? `${Math.round(viewerZoom * 100)}%` : "auto",
                    height: "auto",
                    display: "block",
                  }}
                />
...
function ModelsLibraryPage({ user, assets, setAssets }) {
  ...
  const setFaceReferenceForAsset = useCallback((assetId, dataUrl, name = "") => {
    if (!assetId) return;
    setAssets((prev) => prev.map((asset) => (
      asset.id === assetId
        ? {
          ...asset,
          faceReferenceDataUrl: dataUrl || "",
          faceReferenceName: name || "",
        }
        : asset
    )));
  }, [setAssets]);

  const applyFaceReferenceFile = useCallback(async (assetId, file) => {
    if (!assetId || !file) return;
    try {
      setFaceUploadError("");
      const dataUrl = await fileToRenderableDataUrl(file);
      await ensureImageWithinMegapixels(dataUrl, PRODUCT_UPLOAD_MAX_MP);
      setFaceReferenceForAsset(assetId, dataUrl, file.name || "face-reference");
    } catch (e) {
      setFaceUploadError(e instanceof Error ? e.message : "顔参照画像の登録に失敗しました。");
    }
  }, [setFaceReferenceForAsset]);
  ...
  return (
    <div className="fade-up">
      ...
      <div className="models-layout" style={{ marginBottom: 16 }}>
              ...
              <div style={{ padding: "8px 9px", borderTop: `1px solid ${C.borderLight}` }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                  {!asset.isGenerating && (
                    <button
                      onClick={() => toggleFavorite(asset.id)}
                      style={{ background: "none", border: "none", color: asset.favorite ? C.gold : C.textSub, fontSize: 11, cursor: "pointer", padding: 0 }}
                    >
                      {asset.favorite ? "★ お気に入り" : "☆ お気に入り"}
                    </button>
                  )}
        ...
        <aside className="models-sidebar" style={{ minWidth: 0 }}>
      ...
      {viewer && viewer.items.length > 0 && typeof document !== "undefined" && createPortal((
        ...
        >
          ...
          >
            ...
            <div style={{ position: "relative", flex: 1, minHeight: 0, background: C.bg, overflow: "hidden" }}>
              <div style={{ height: "100%", minHeight: 0, display: "grid", gridTemplateColumns: "minmax(0,1fr) 280px" }}>
                <div
                  style={{
                    position: "relative",
                    height: "100%",
                ...
                <aside style={{ borderLeft: `1px solid ${C.borderLight}`, background: C.surface, padding: 14, overflowY: "auto" }}>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      <div style={{ marginBottom: 8 }}>
                        <select
                          ...
                          onChange={(e) => setModelSaveFormat(e.target.value)}
                          style={{
                            border: `1px solid ${C.border}`,
                            background: C.bg,
                            color: C.text,
                      ...
                      <Btn
                        size="sm"
                        variant="secondary"
                        onClick={() => downloadModelAsset(viewer.items[viewer.index], modelSaveFormat)}
                      >
                        保存
                      </Btn>
                      <div style={{ marginTop: 8 }}>
                  ...
                  <p style={{ fontSize: 11, color: C.textMid, marginBottom: 8 }}>顔参照画像</p>
                  <input
                    ref={faceInputRef}
                    type="file"
                    accept="image/*,.heic,.heif"
                    ...
                    onChange={(e) => {
                      ...
                      const target = viewer.items[viewer.index];
                      if (target && files[0]) {
                        void applyFaceReferenceFile(target.id, files[0]);
                      }
                      e.target.value = "";
                    }}
                  />
                  <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                    <Btn size="sm" variant="secondary" onClick={() => faceInputRef.current?.click()}>登録</Btn>
                    <Btn
                      size="sm"
                      variant="ghost"
                      onClick={() => {
                        const target = viewer.items[viewer.index];
                        if (target) setFaceReferenceForAsset(target.id, "", "");
                      }}
                    >
                      クリア
                    </Btn>
                  </div>
                  {viewer.items[viewer.index]?.faceReferenceDataUrl ? (
                    <div style={{ border: `1px solid ${C.borderLight}`, background: C.bg, padding: 8 }}>
                      <img
                        src={viewer.items[viewer.index]?.faceReferenceDataUrl}
                        alt="face reference"
                        style={{ width: "100%", aspectRatio: "1/1", objectFit: "cover", marginBottom: 6 }}
                      />
                      <p style={{ fontSize: 10, color: C.textSub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {viewer.items[viewer.index]?.faceReferenceName || "face-reference"}
                      </p>
                    </div>
                  ) : (
                    <p style={{ fontSize: 10, color: C.textSub }}>未登録</p>
                  )}
                  {faceUploadError && (
                    <p style={{ marginTop: 8, fontSize: 11, color: C.red }}>{faceUploadError}</p>
                  )}
                </aside>
              </div>
            </div>
...
export default function App() {
  ...
  const refreshData = useCallback(async () => {
    ...
    if (currentUser) {
      ...
      try {
        ...
        if (localHasAssets) {
          const hasDataUrl = (asset) => (
            String(asset?.dataUrl || "").startsWith("data:")
            || String(asset?.outputUrl || "").startsWith("data:")
            || String(asset?.faceReferenceDataUrl || "").startsWith("data:")
          );
          const hasHeavyLocal = [...(localLib.studio || []), ...(localLib.models || []), ...(localHydratedProducts || [])].some(hasDataUrl);
          const hasMissingRemote = (remoteArr = [], localArr = []) => {
            const remoteIds = new Set((remoteArr || []).map((asset) => asset.id));
            return (localArr || []).some((asset) => !remoteIds.has(asset.id));
          };
          const needsMigration = hasHeavyLocal
            || hasMissingRemote(remoteLib.studio, localLib.studio)
            || hasMissingRemote(remoteLib.models, localLib.models)
            || hasMissingRemote(remoteLib.products, localHydratedProducts);
          if (needsMigration) {
        ...
        setProductAssets(mergeProductAssets(remoteLib.products));
      } catch {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        await saveAssetLibrary(user.id, {
          studio: studioAssets,
          models: modelAssets,
        ...
        writeAssetLibrary(user.id, {
          studio: studioAssets,
          models: modelAssets,
          products: stripProductAssetsForMeta(productAssets),
        });
        await persistProductAssetsToDb(user.id, productAssets);
      }
    })();
  ...
  const pages = {
    history: <HistoryPage user={user} jobs={jobs} onRefresh={refreshData} />,
    products: (
      <ProductsLibraryPage
        user={user}
    ...
    edit: <EditPage jobs={jobs} user={user} studioAssets={studioAssets} modelAssets={modelAssets} />,
    studio: (
      <AssetLibraryPage
        title="スタジオ"
      ...
      <div style={{ display: "flex", minHeight: "100vh", fontFamily: SANS }}>
        ...
        <main style={{
          ...
          <div style={{ display: page === "upload" ? "block" : "none" }}>
            <UploadPage user={user} jobs={jobs} onDataRefresh={refreshData} studioAssets={studioAssets} modelAssets={modelAssets} productAssets={productAssets} />
          </div>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <p style={{ fontFamily: SERIF, fontSize: 24, color: C.text }}>クレジット履歴</p>
            ...
            {creditHistory.length === 0 ? (
              ...
              <div style={{ border: `1px solid ${C.border}` }}>
                {creditHistory.map((event) => {
                  ...
                  return (
                    <div key={event.id} style={{ display: "grid", gridTemplateColumns: "170px 1fr 140px", gap: 10, alignItems: "center", padding: "10px 12px", borderTop: `1px solid ${C.borderLight}` }}>
